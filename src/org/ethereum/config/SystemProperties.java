 ...
 *
 * The properties are taken from different sources and merged in the following
 * order (the config option from the next source overrides option from
 * previous): - resource ethereumj.conf : normally used as a reference config
 * with default values and shouldn't be changed - system property : each config
 * entry might be altered via -D VM option - [user dir]/config/ethereumj.conf -
 * config specified with the -Dethereumj.conf.file=[file.conf] VM option - CLI
 * options
 *
...
public class SystemProperties {
	...
	public SystemProperties(Config apiConfig, ClassLoader classLoader) {
		try {
			...
			config = apiConfig.withFallback(cmdLineConfigFile).withFallback(testUserConfig).withFallback(testConfig)
					.withFallback(userDirConfig).withFallback(userConfig).withFallback(cmdLineConfigRes)
					.withFallback(referenceConfig);

			logger.debug("Config trace: "
					+ config.root().render(ConfigRenderOptions.defaults().setComments(false).setJson(false)));

			config = javaSystemProperties.withFallback(config).resolve(); // substitute
																			// variables
																			// in
																			// config
																			// if
	...
	/**
	 * Puts a new config atop of existing stack making the options in the
	 * supplied config overriding existing options Once put this config can't be
	 * removed
	 *
	 ...
	 *
	 * @param cliOptions
	 *            - command line options to take presidency
	 */
	...
	// if (genesisJson.getConfig() != null &&
	// genesisJson.getConfig().isCustomConfig()) {
	// blockchainConfig = new JsonNetConfig(genesisJson.getConfig());
	// } else {
	// if (config.hasPath("blockchain.config.name") &&
	// config.hasPath("blockchain.config.class")) {
	// throw new RuntimeException("Only one of two options should be defined:
	// 'blockchain.config.name' and 'blockchain.config.class'");
	// }
	// if (config.hasPath("blockchain.config.name")) {
	...
	// try {
	// Class<? extends BlockchainNetConfig> aClass = (Class<? extends
	// BlockchainNetConfig>) classLoader.loadClass(className);
	// blockchainConfig = aClass.newInstance();
	// } catch (ClassNotFoundException e) {
	// throw new RuntimeException("The class specified via
	// blockchain.config.class '" + className + "' not found", e);
	// } catch (ClassCastException e) {
	// throw new RuntimeException("The class specified via
	// blockchain.config.class '" + className + "' is not instance of
	// org.ethereum.config.BlockchainForkConfig", e);
	// } catch (InstantiationException | IllegalAccessException e) {
	// throw new RuntimeException("The class specified via
	// blockchain.config.class '" + className + "' couldn't be instantiated
	// (check for default constructor and its accessibility)", e);
	// }
	// }
	// }
	//
	// if (genesisJson.getConfig() != null &&
	// genesisJson.getConfig().headerValidators != null) {
	// for (GenesisConfig.HashValidator validator :
	// genesisJson.getConfig().headerValidators) {
	// BlockHeaderValidator headerValidator = new BlockHeaderValidator(new
	// BlockCustomHashRule(ByteUtil.hexStringToBytes(validator.hash)));
...
//		return discoveryEnabled == null ? config.getBoolean("peer.discovery.enabled") : discoveryEnabled;
		return discoveryEnabled == null ? true : discoveryEnabled;
	}
...
//		return config.getInt("peer.discovery.touchPeriod");
	...
	// return config.hasPath("peer.p2p.version") ?
	// config.getInt("peer.p2p.version") : P2pHandler.VERSION;
	// }
	//
	// @ValidateMe
	// public int rlpxMaxFrameSize() {
	// return config.hasPath("peer.p2p.framing.maxSize") ?
	// config.getInt("peer.p2p.framing.maxSize") : MessageCodec.NO_FRAMING;
	// }
...
//		return databaseReset == null ? config.getBoolean("database.reset") : databaseReset;
		return databaseReset == null ? false : databaseReset;
	}
	...
	// if (nodeId.length != 64) {
	// throw new RuntimeException("Invalid config nodeId '" + nodeId + "' at " +
	// configObject);
	// }
	// } else {
	...
	// } else {
	// throw new RuntimeException("Either nodeId or nodeName should be
	// specified: " + configObject);
	// }
	// }
	// n = new Node(nodeId, ip, port);
	// } else {
	// throw new RuntimeException("Unexpected element within 'peer.active'
...
//		return databaseDir == null ? config.getString("database.dir") : databaseDir;
		return databaseDir == null ? "database" : databaseDir;
	}
...
//		return vmTrace == null ? (vmTrace = config.getBoolean("vm.structured.trace")) : vmTrace;
		return vmTrace == null ? (vmTrace = false) : vmTrace;
	}
...
//		return genesisInfo == null ? config.getString("genesis") : genesisInfo;
		return genesisInfo == null ? "frontier.json" : genesisInfo;
	}
...
//		return config.getString("crypto.hash.alg512");
	...
	// if (genesis == null) {
	// genesis = GenesisLoader.parseGenesis(getBlockchainConfig(),
	// getGenesisJson());
	// }
	// return genesis;
	// }
